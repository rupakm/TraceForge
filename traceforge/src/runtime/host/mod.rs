/// A host abstraction: hosts are groups of threads running on a machine
/// The host keeps track of machine-related stuff like network ports in use,
/// (eventually) the clock, etc.
/// It also allows crashing and rebooting, in order to simulate faults
use crate::net::envelope::{hex, Datagram, Envelope, Protocol, Segment, Syn};
use crate::net::{Channel, MustUdpController, SocketPair, TcpListener, UdpSocket};
use crate::send_msg;
...
struct UdpBind {
    ...
    queue: crate::net::Channel<Datagram, SocketAddr>,
}
...
impl Udp {
    ...
    pub(crate) fn bind(&mut self, addr: SocketAddr) -> io::Result<UdpSocket> {
        ...
        let udplistener = crate::thread::Builder::new()
            .name(format!("{}", addr))
            .spawn_daemon(move || {
                let mut buffer = VecDeque::<(Datagram, SocketAddr)>::new();
                let mut capacity = capacity;
                let mut waiter: Option<ThreadId> = None;
                loop {
                    let r = crate::recv_msg_block();
                    match r {
                        MustUdpController::UdpData(dg, addr) => {
                            if capacity > 0 {
                                buffer.push_back((dg, addr));
                                capacity -= 1;
                                if let Some(tid) = waiter.take() {
                                    send_msg(tid, buffer.pop_front());
                                    capacity += 1;
                                }
                            }
                            // otherwise drop the packet
                        }
                        MustUdpController::UdpGetRequest(tid) => {
                            if let Some(b) = buffer.pop_front() {
                                send_msg(tid, b);
                                capacity += 1;
                            } else {
                                assert!(waiter == None, "at most one waiter expected");
                                waiter = Some(tid);
                            }
                        }
                    }
            ...
            .unwrap();
        let udpchannel = Channel::new(udplistener.thread().id());
        let bind = UdpBind {
            bind_addr: addr,
            queue: udpchannel.clone(),
        };
        ...
        Ok(UdpSocket::new(addr, udpchannel))
    }
    ...
    fn buffer(&mut self, seq: u64, segment: SequencedSegment) -> Result<(), Protocol> {
        ...
        while self.buf.contains_key(&(self.recv_seq + 1)) {
            ...
            self.sender.try_send(segment).map_err(|e| match e {
                Disconnected(_) => Protocol::Tcp(Segment::Rst),
...
mod test {
    ...
    use crate::runtime::host::{Host, HostTimer};
